...
export class SchoolLobbyScene extends Phaser.Scene {
  ...
  async create(data) {
    ...
    const studentObj = map.getObjectLayer("Student1")?.objects[0];
    const studentX = studentObj?.x ?? 310;
    const studentY = studentObj?.y ?? 280;

    this.student1 = this.physics.add.sprite(studentX, studentY, "student1");
    this.student1.setDepth(5);
    this.student1.setScale(1.5); // FIX: Match Sakura/Mother scale
    ...
    // Set accurate hitbox for 1.5 scale (standard 24x32 effective)
